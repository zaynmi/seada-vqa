...
def path_for(train=False, val=False, test=False, question=False, trainval=False, answer=False, vqacp=False, sea=False, eda=False, iq=False):
    ...
    if not vqacp:
        ...
        if question:
            ...
            if sea:
                fmt = 'v2_{0}_{1}_{2}_questions_adv.json'
            if eda:
                fmt = 'v2_{0}_{1}_{2}_questions_eda.json'
            if iq:
                fmt = 'v2_{0}_{1}_{2}_questions_iq.json'
        else:
            if test:
                # just load validation data in the test=answer=True case, will be ignored anyway
                split = 'val2014'
            if eda:
                fmt = 'v2_{1}_{2}_annotations_eda.json'
            elif iq:
                fmt = 'v2_{1}_{2}_annotations_iq.json'
            else:
        ...
        if question:
            fmt = 'vqacp/vqacp_v2_{0}_questions.json'
            if sea:
                fmt = 'vqacp/vqacp_v2_{0}_questions_adv.json'
            if eda:
                fmt = 'vqacp/vqacp_v2_{0}_questions_eda.json'
            if iq:
                fmt = 'vqacp/vqacp_v2_{0}_questions_iq.json'
        else:
            if test:
                # just load validation data in the test=answer=True case, will be ignored anyway
                split = 'test'
            if eda:
                fmt = 'vqacp/vqacp_v2_{0}_annotations_eda.json'
            elif iq:
                fmt = 'vqacp/vqacp_v2_{0}_annotations_iq.json'
            else:
                fmt = 'vqacp/vqacp_v2_{0}_annotations.json'
